...
def prepare_training_data(seqs_x, seqs_y_c, seqs_y_cw,  maxlen_chunk=None, maxlen_cw=None, n_words_src=30000,
    ...
    for idx, [s_x, s_y_c, s_y_cw] in enumerate(zip(seqs_x, seqs_y_c, seqs_y_cw)):
        ...
        chunk_indicator[lengths_y[idx], idx] = 1.0
...
# build a training model
def build_alignment(tparams, options):
    opt_ret = dict()
    ...
    # predict the chunk tag and chunk boundary
    ...
    predict_chunk = tensor.argmax(probs_chunk,1)
    ...
    predict_boundary = tensor.argmax(probs_boundary, 1)


    # predict the chunk tag and chunk boundary end!




    chunk_alpha = chunk_alpha
    align = chunk_alpha.reshape([chunk_alpha.shape[0], chunk_alpha.shape[2]])
    # weights (alignment matrix)
    print 'Building alignment..',
    inps = [x, x_mask, y_chunk, y_chunk_words, y_mask, chunk_indicator]
    outs = [align, predict_chunk, predict_boundary]
    f_alignment = theano.function(inps, outs, name='f_alignment', profile=profile)
    print 'Done'
    ...
    for eidx in xrange(max_epochs):
        ...
        for x, y_chunk, y_cw in train:
            ...
            if numpy.mod(uidx, sampleFreq) == 0:
                ...
                saveto_test = ""
                for test, test_chunk, test_cw in valid:
                    print '%d testing samples' % len(test)
                    test, test_mask, test_c, test_cw, chunk_indicator, y_mask = \
                    ...
                    print len(test)
                    for test_j in xrange(test.shape[1]):
                        # print test_j
                        stochastic = True
                        # print test[test_j]
                        # print test[:, test_j]
                        sample, sample_boundary, sanple_chunk, score = gen_sample(tparams, f_init, f_next_chunk,
                                                                                  f_next_word,
                                                                                  test[:, test_j][:, None],
                                                                                  ...
                                                                                  argmax=False)
                        saveto_test += 'Source '
                        saveto_test += str(test_j)
                        saveto_test += ' : '
                        for vv in test[:, test_j]:
                            if vv == 0:
                                break
                            if vv in worddicts_r[0]:
                                saveto_test += worddicts_r[0][vv]
                                saveto_test += ' '
                            else:
                                saveto_test += 'UNK'
                                saveto_test += ' '
                        ...
                        ci = 0
                        # print y_chunk[: , jj]
                        for chunk_index, word_index in zip(test_c[:, test_j], test_cw[:, test_j]):
                            if word_index == 0:
                                break
                            if chunk_index in worddict_r_chunk and chunk_index != 1:  # not NULL
                                saveto_test += '|'
                                saveto_test += ' '
                                saveto_test += worddict_r_chunk[chunk_index]
                                saveto_test += ' '
                            if word_index in worddicts_r[1]:
                                saveto_test += worddicts_r[1][word_index]
                                saveto_test += ' '
                            else:
                                saveto_test += 'UNK'
                                saveto_test += ' '
                        ...
                        for vv in ss:
                            ...
                            if vv < 0:
                                ...
                                saveto_test += '|'
                                saveto_test += ' '
                                saveto_test += worddict_r_chunk[vv]
                                saveto_test += ' '
                                continue
                            if vv in worddicts_r[1]:
                                saveto_test += worddicts_r[1][vv]
                                saveto_test += ' '
                            else:
                                saveto_test += 'UNK'
                                saveto_test += ' '
                        saveto_test += '\n'
                output_train_r = open('results', 'w')
                output_train_r.write(saveto_test)
